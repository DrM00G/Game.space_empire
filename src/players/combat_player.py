...
class CombatPlayer(Player):
                    ...
                    if choice == 9:
                        if planet.base_status == 0:
                            ...
                            self.playerCP = self.playerCP - army_choices[
                                choice][1]
                            self.game.boolean_print("made unit:" + self.units[
                                len(self.units) - 1].name + " " +
                                                    str(len(self.units)))
                            print("made unit:" + self.units[
                                len(self.units) - 1].name + " " +
                                                    str(len(self.units)))
                        ...
                        self.game.boolean_print("made unit:" + self.units[len(
                            self.units) - 1].name + " " + str(len(self.units)))
                        print("made unit:" + self.units[len(
                            self.units) - 1].name + " " + str(len(self.units)))
                        self.Game.board.add_unit_to_board(
    ...
    def unit_upgrade(self):
        if self.game.predetermined_roll == None:
            ...
            upgrade_catagory = self.game.predetermined_roll[0]
            self.game.predetermined_roll.remove(
                self.game.predetermined_roll[0])
        print(str(upgrade_catagory))
        ...
        if upgrade_catagory == 1:  #offense
            if self.attack_technology < 1 and self.playerCP - (
                ...
                self.game.boolean_print("Player " + str(self.player_num) +
                                        ": ^Attack Technology: " +
            ...
            if self.ship_yard_technology < 6 and self.playerCP - (
                ...
                self.playerCP = self.playerCP - (
                    ((self.ship_yard_technology-1) * 5) + 10)
                print((
                    ((self.ship_yard_technology-1) * 5) + 10))
