...
class CombatPlayer(Player):
          ...
          if self.playerCP >= army_choices[choice][1] and hull_capacity >= army_choices[choice][2]*2:
            if choice == 9:
              if planet.base_status == 0:
                ...
                self.game.boolean_print("made unit:"+self.units[len(self.units)-1].name+" "+str(len(self.units)))
    ...
    def unit_upgrade(self):
      ...
      if upgrade_catagory == 1:#offense
        if self.attack_technology < 1 and self.playerCP - ((self.attack_technology*10)+20) >= 0:
          self.playerCP = self.playerCP - ((self.attack_technology*10)+20)
          self.attack_technology = self.attack_technology + 1
          self.game.boolean_print("Player "+str(self.player_num)+": ^Attack Technology: "+str(self.attack_technology-1)+"->"+str(self.attack_technology))
        ...
        if self.defense_technology < 3 and self.playerCP - ((self.defense_technology*10)+20) >= 0:
          ...
          self.game.boolean_print("Player "+str(self.player_num)+": ^Defense Technology: "+str(self.defense_technology-1)+"->"+str(self.defense_technology))

      elif upgrade_catagory == 2:#sped
        if self.speed_technology < 6 and self.playerCP - ((self.speed_technology*10)+20) >= 0:
          self.playerCP = self.playerCP - ((self.speed_technology*10)+20)
          self.speed_technology = self.speed_technology + 1
          self.game.boolean_print("Player "+str(self.player_num)+": ^Speed Technology: "+str(self.speed_technology-1)+"->"+str(self.speed_technology))
       ...
       if self.ship_yard_technology < 6 and self.playerCP - ((self.ship_yard_technology*5)+10) >= 0:
        ...
        self.game.boolean_print("Player "+str(self.player_num)+": ^Ship Size Technology: "+str(self.ship_yard_technology-1)+"->"+str(self.ship_yard_technology))
